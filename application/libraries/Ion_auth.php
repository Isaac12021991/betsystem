...
class Ion_auth
{
			...
			public function get_nombre_usuario()
	{
		...
		return $usuario->first_name;
			...
			public function get_apellido_usuario()
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->last_name;
	}
	...
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->co_partner;
	}

			public function tipo_empresa()
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->nb_tipo_empresa;
	}

			public function empresa()
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->nb_empresa;
	}
	...
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->nb_foto_perfil;
	}

				public function ubicacion_estado()
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->nb_estado;
	}


			function general_role_lu_users_group($var)
	{
		...
		$sql="SELECT * 
		from lu_users as a
		join lu_users_groups as b on b.user_id = a.id 
		where a.id = '$usuario->id' and b.group_id = '$value'";
        ...
        if ($query->num_rows() > 0) {
			...
			}else
			{
				...
				public function in_verificado()
	{
		$usuario=$this->ion_auth_model->user()->row();
		return $usuario->in_verificado;
	}
						...
						function in_cedula($nu_cedula)
	{


		$usuario=$this->ion_auth_model->user()->row();
		if($nu_cedula == $usuario->nu_cedula):
		...
		function in_empresa_activado()
	{

		$usuario=$this->ion_auth_model->user()->row();
		$sql="SELECT * 
		from j049t_empresas_farmaceuticas_todas as a
		where a.id = '$usuario->co_partner' and a.in_activo = true and a.in_validado = true limit 1";
        $query=$this->db->query($sql);
        if($query->num_rows() > 0): return true; else: false; endif;
	}

			function info_partner_todo()
	{

		$usuario=$this->ion_auth_model->user()->row();
		$sql="SELECT * 
		from j049t_empresas_farmaceuticas_todas as a
		where a.id = '$usuario->co_partner' and a.in_activo = true limit 1";
        $query=$this->db->query($sql);
        return $query->row();
	}
					...
					function promedio_reputacion_empresa($nb_ambito, $co_partner)
	{
		
		$usuario=$this->ion_auth_model->user()->row();
		$sql="SELECT COUNT(a.id) as ca_transacciones,
		AVG(a.ca_calificacion) as ca_promedio_reputacion,
				(CASE WHEN AVG(a.ca_calificacion) < 2.5 
				THEN 'BAJO' 
				WHEN AVG(a.ca_calificacion) > 2.5 and AVG(a.ca_calificacion) < 4 
				THEN 'MEDIO'
				WHEN AVG(a.ca_calificacion) > 4 
				THEN 'ALTO' END) as nb_reputacion
				FROM j079t_usuario_partner_calificado as a
                WHERE a.co_partner_calificado = '$co_partner' and a.in_activo = true and a.nb_ambito = '$nb_ambito'";
        $query=$this->db->query($sql);
        $get_reputacion = $query->row();
        if ($get_reputacion->ca_transacciones > 5): return $query->row(); else: return false; endif;
	}

					function promedio_reputacion_usuario($nb_ambito, $co_usuario)
	{
		...
		AVG(a.ca_calificacion) as ca_promedio_reputacion,
				...
				THEN 'ALTO' END) as nb_reputacion
 				FROM j079t_usuario_partner_calificado as a
                WHERE a.co_usuario_calificado = '$co_usuario' and a.in_activo = true and a.nb_ambito = '$nb_ambito'";
        $query=$this->db->query($sql);
        $get_reputacion = $query->row();
        if ($get_reputacion->ca_transacciones > 5): return $query->row(); else: return false; endif;
	}
			...
			function saldo()
	{
		...
		where a.co_usuario = '$usuario->id' and a.co_partner = '$usuario->co_partner' and a.in_activo = true limit 1";
        $query=$this->db->query($sql);
        $query = $query->row();
        return $query;
