...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $co_partner)
	{
		...
		$query = $this->db->select($this->identity_column. ', email, id, password, active, last_login, co_partner')
		                  ->where($this->identity_column, $identity)
		                  ->where('co_partner', $co_partner)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if(!$this->verificar_partner($co_partner)):
		return FALSE;
		endif;
	...
	**/
	...
	function verificar_partner($co_partner) {
		// just return the string IP address now for better compatibility
					$sql = "SELECT *
		FROM j049t_empresas_farmaceuticas_todas as a
		where a.id = $co_partner and a.in_activo = true";
        $query_partner = $this->db->query($sql);        

        if ($query_partner->num_rows() === 0)
		{

			$this->set_message('Club Cerrado');
			return FALSE;
